...
class BackblazeTest extends TestCase
{
    ...
    {
        $this->root = '/root';
        $key = $_SERVER['BB_ACCESS_KEY'] ?? '';
        $secret = $_SERVER['BB_SECRET'] ?? '';
        $bucket =  $_SERVER['BB_BUCKET'] ?? 'utopia-storage-tests';

        print $bucket;

        $this->object = new Backblaze($this->root, $key, $secret, $bucket, Backblaze::US_WEST_2, Backblaze::ACL_PUBLIC_READ);
    ...
    {
        $this->assertEquals($this->object->getName(), 'Backblaze Storage');
    }

    public function testDescription()
    {
        $this->assertEquals($this->object->getDescription(), 'Backblaze Storage');
    }
